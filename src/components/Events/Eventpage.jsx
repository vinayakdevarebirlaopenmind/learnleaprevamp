      ...
      <h2 className="common-heading">Course Events</h2>
        ...
        <div className="learnleap-events-grid">
          {filteredEvents.map((event) => (
            ...
            >
              <img src={event.image} alt={event.title} className="learnleap-events-image" />
              <h3>{event.title}</h3>
              <p>{new Date(event.date).toLocaleDateString()}</p>
              <p>{event.attendees} attendees</p>
              <span className={`learnleap-events-tag ${event.status}`}>{event.status}</span>
        ...
        {openDialog && selectedEvent && (
          <div className="learnleap-events-dialog-overlay">
            <div className="learnleap-events-dialog">
              <button className="learnleap-events-close-btn" onClick={() => setOpenDialog(false)}>
                <MdClose />
              </button>
              <img src={selectedEvent.image} alt={selectedEvent.title} className="learnleap-events-dialog-image" />
